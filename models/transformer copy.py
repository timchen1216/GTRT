...
class LinearAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        v = rearrange(v, "b t (g c) -> b g t c", g=self.num_heads)
        if mask is not None:
            assert mask.ndim == 1 or mask.ndim == 2  # [B, T] or [B*T]
            if mask.ndim == 1:
                mask = mask.view(bsz, -1)  # reshape to [B, T]
            # 將 mask 擴展到正確的維度 [B, num_heads, T, 1]
            mask = rearrange(mask, "b t -> b 1 t 1").expand(-1, self.num_heads, -1, 1)
            k.masked_fill_(mask == 0, 0)  # mask 中的 0 表示要遮蔽的部分
...
class EncoderBlock(nn.Module):
    ...
    mask : (batch_size, sequence_length) or (batch_size, sequence_length, sequence_length)
        Attention mask, where:
        - If 2D: (batch_size, sequence_length) indicates which tokens to attend to
        - If 3D: (batch_size, sequence_length, sequence_length) indicates valid attention pairs
        In both cases, 1 indicates attention is allowed, 0 indicates attention is masked out
...
class DecoderBlock(nn.Module):
    ...
    ----------
    dim : int
        Input and output dimension of queries and output
    hidden_dim : int
        Hidden dimension in feed-forward network
    memory_dim : int, optional
        Dimension of memory (key/value). If None, uses dim

    Input shapes
    -----------
    x : (batch_size, sequence_length, dim)
        Input sequence
    memory : (batch_size, memory_sequence_length, memory_dim)
        Memory sequence from encoder
    mask : (batch_size, sequence_length) or (batch_size, sequence_length, sequence_length)
        Attention mask for self-attention
    memory_mask : (batch_size, sequence_length, memory_sequence_length)
        Attention mask for memory attention
    ...
    def __init__(self, dim, hidden_dim, memory_dim=None, num_heads=8, dropout_prob=0.1):
        ...
        self.mem_attn = MultiHeadAttention(
            dim, memory_dim, memory_dim, num_heads, dropout_prob
        )
        self.ffn = PositionwiseFFN(dim, hidden_dim, dropout_prob)
        self.dropout = nn.Dropout(dropout_prob)
        self.ln1 = RMSNorm(dim)
...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, x):
        _, tsz, _ = (
            x.shape
        )  # 這裡期望輸入是 3D tensor: [batch_size, sequence_length, feature_dim]
        return x + self.emb[:tsz, :]  # self.emb 的形狀是 [max_length, dim]
