...
from precompute_tracklet_info import precompute_tracklet_info
...
def train_tracklet_graph_model(tracklet_graph_model, tracklet_data, optimizer, device):
    ...
    for batch_idx, batch in progress_bar:
        ...
        loss = (
            sum([l.mean() for l in loss_rec])
            + sum([l.mean() for l in loss_trip])
            + sum([l.mean() for l in loss_BCE])
        )
...
def validate_tracklet_graph_model(tracklet_graph_model, val_data, device):
    ...
    for epoch in range(num_epochs):
        try:
            ...
            val_loss, val_loss_rec, val_loss_trip, val_loss_BCE = (
                validate_tracklet_graph_model(tracklet_graph_model, val_data, device)
            )
            print(
