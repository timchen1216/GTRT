...
def batch_test_tracklet_graph(
    ...
    for b in range(len(batch_data)):
        ...
        obj_ids = batch_data[b]["obj_ids"][0].to(device).float()
        ...
        while True:
            ...
            if (
                tracklet_data["A"].shape[0] < 2
                or torch.max(tracklet_data["A"]).item() < 1
            ):
                t_fr += T_tracklet_stride
                if end_flag == 1:
                    break
